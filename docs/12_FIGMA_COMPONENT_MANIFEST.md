...
| 2026-08-13 | **Visual Approval（經使用者授權，由 team-lead 驗收核准）** — 以 `get_screenshot` 取得實際渲染逐一檢視 | `BTN_DEAL`(`9:17`) 推進為 `1.1.0`、`PANEL_ACTION_BAR`(`21:38`) 與 `VALUE_TOTAL`(`21:62`) 推進為 `1.0.0`，三者 `status` 改為 `APPROVED_PENDING_GODOT_SYNC`。**核准權限來源：使用者於 2026-08-13 明示授權「驗收過後即可通過」，非使用者本人檢視。** 檢視內容：`PANEL_ACTION_BAR` 五個 variant 與 `RoundController.legal_actions()` 實際輸出逐一對應（`DEAL` / `HIT·STAND·DOUBLE·SURRENDER` / `HIT·STAND` / `NEXT ROUND` / 全灰 blocking），SURRENDER 走 danger 紅、其餘走 `action/primary` 金，blocking 為整排變灰而非清空（與 `docs/03:126` 一致）；`NEXT ROUND` 確認走 `Label` text property 而非新元件。`VALUE_TOTAL` 三態配色正確（HARD=`text/primary` 白、SOFT=`text/secondary` 灰綠、BUST=`result/bust` 深紅），透明底無多餘面板色。**未涵蓋**：`AGENTS.md` §9 No-Guessing 清單中的內容尺度與 progression 方向仍為使用者專屬決定，不在本次授權範圍。 |
...
| `BTN_DEAL` | Deal Button | <https://www.figma.com/design/vufbRMFF4rpBt6W1jedHxb> | `9:17` | `1.1.0` | `native_control` | `res://ui/components/deal_button.tscn` | `State`（variants）+ `Label`（text property `Label#24:0`，預設 `DEAL`，另一合法值 `NEXT ROUND`） | `2026-08-13` | `APPROVED_PENDING_GODOT_SYNC` |
...
| `VALUE_TOTAL` | Hand Total | <https://www.figma.com/design/vufbRMFF4rpBt6W1jedHxb> | `21:62` | `1.0.0` | `native_control` | `res://ui/components/value_display.tscn` | `State` = `Hard / Soft / Bust`（variants）+ `Value`（text property） | `2026-08-13` | `APPROVED_PENDING_GODOT_SYNC` |
...
| `PANEL_ACTION_BAR` | Action Bar | <https://www.figma.com/design/vufbRMFF4rpBt6W1jedHxb> | `21:38` | `1.0.0` | `native_control` | `res://ui/components/action_bar.tscn` | `Player State` = `Betting / PlayerTurnFirst / PlayerTurnDecided / RoundEnd / Blocking` | `2026-08-13` | `APPROVED_PENDING_GODOT_SYNC` |
